...
import com.joe.task.util.HasuraUtil;
import com.joe.task.util.HasuraUtil.RemoteSchema;
import com.joe.task.util.HasuraUtil.SchemaHealthStatus;
...
public class HasuraController {
    ...
    @Autowired
    private HasuraProperties hasuraProperties;

    @PostConstruct
    public void init() {
        HasuraUtil.init(hasuraProperties);
    }
    ...
    public Result addRemoteSchema(@RequestBody AddSchemaRequest request) {
        ...
        try {
            boolean success = HasuraUtil.addRemoteSchema(
                request.getEnv(), 
    ...
    public Result deleteRemoteSchema(@RequestBody DeleteSchemaRequest request) {
        ...
        try {
            boolean success = HasuraUtil.deleteRemoteSchema(request.getEnv(), request.getSchemaName());
    ...
    public Result deleteRemoteSchemaByNamespace(@RequestBody DeleteSchemaByNamespaceRequest request) {
        ...
        try {
            boolean success = HasuraUtil.deleteRemoteSchema(
                request.getEnv(), 
                request.getNamespace(), 
                request.getServiceName()
            );
    ...
    public Result searchSchemas(@RequestParam String env, @RequestParam String keyword)
    {
        ...
        // 其他环境走主逻辑
        try {
            List<RemoteSchema> schemaList = HasuraUtil.listRemoteSchemas(env, keyword);
    ...
    public Result checkSchemaHealth(
        ...
        try {
            List<SchemaHealthStatus> allStatuses = HasuraUtil.checkAllRemoteSchemasHealth(env);
    ...
    public Result checkAllSchemasHealth(@PathVariable String env) {
        ...
        try {
            List<SchemaHealthStatus> healthStatuses = HasuraUtil.checkAllRemoteSchemasHealth(env);
    ...
    public Result getHasuraEnvOptions() {
        try {
            // 从配置中获取所有环境
            List<Object> options = hasuraProperties.getEnvironments().keySet().stream()
                    .map(envKey -> {
                        Result option = Result.ok();
                        option.put("value", envKey);
                        option.put("label", envKey.toUpperCase());
                        return option;
                    })
    ...
    public Result refreshRemoteSchema(@RequestBody RefreshSchemaRequest request) {
        ...
        try {
            boolean success = HasuraUtil.refreshRemoteSchema(request.getEnv(), request.getSchemaName());
            if (success) {
